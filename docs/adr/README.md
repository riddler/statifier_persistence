...
| [0012](0012-retention-and-retirement.md) | Retention and retirement: three things pin a chart (an `:active` execution, a durable child's linkage pin while its parent is `:active`, a registered pin source); zero pins is retirable; `retire_chart` refuses with every count by name or tombstones the row and nulls the blobs in one transaction; a retired hash answers its own error on both chart doors and is never revived by a save; no clock | proposed |
