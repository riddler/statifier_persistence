...
rather than discovering them at the far end of an execution.
...
|---|---|---|
| `lock_execution/3` | `SELECT pg_advisory_xact_lock(hashtextextended(...))` plus `FOR UPDATE` | The backend does not parse it. The callback raises |
| `list_executions_by_metadata/2`, `list_execution_states_by_metadata/2` | `jsonb` containment (`@>`) with a `-> ... ->>` extraction | The backend does not parse it, so the callbacks never issue it: each consults `supports_metadata?/1` first and answers `{:error, :metadata_unsupported}`, called directly or through the facade |
...
The execution metadata *column* is not Postgres-only, and neither is
anything else in the loop: charts, positions, execution records, the identity
guard, the
...
Per-execution ordering is a strategy, not a hardcoded lock
...
delegates to the adapter's `lock_execution/3` - the Postgres SQL above. On
another backend, pass your own strategy instead, on every
`StatifierPersistence.Executions` call and on `StatifierPersistence.Driver.new/3`:

```elixir
defmodule MyApp.ExecutionLock do
  @behaviour StatifierPersistence.Serialization

  @impl StatifierPersistence.Serialization
  def with_execution(_config, execution_id, fun) do
    MyApp.SingleWriter.exclusively(execution_id, fun)
  end
end
...
driver =
  StatifierPersistence.Driver.new(store, machine,
    ...
    serialization: {MyApp.ExecutionLock, nil}
  )
```

There are two honest shapes for that strategy:

1. **A real exclusion the host already owns.** A job queue with a
   per-execution unique key, a single consumer per execution id, a distributed
   lock service, a `GenServer` registry keyed by execution id in a single-node
   deployment. The
   guarantee the behaviour asks for is documented on
   `c:StatifierPersistence.Serialization.with_execution/3`: for one
   `execution_id`, two bodies never overlap, and a body that finishes before another starts is
...
2. **A pass-through, plus a single writer by construction.** `fn _c, _id,
   ...
   only when nothing in your deployment can deliver two events to one
   execution concurrently - one node, one worker, no retries that can overlap
   a live delivery.

What you must not do is keep the default. `AdapterLock` on a non-Postgres
repo does not refuse; it reaches `lock_execution/3` and raises,
mid-execution, after
effects may already have gone out.

## What declining costs

Serialization is what makes the fetch-to-persist tail atomic per execution.
With a pass-through, two concurrent deliveries to one execution both load the
same
...
- **Durable subcharts and fan-out** (ADR-0008). Starting a child execution
  needs
...
- **Listing executions by host scope.** `Storage.list_executions_by_metadata/2`
  answers `{:error, :child_listing_unsupported}` and
  `Storage.list_execution_states_by_metadata/2` answers
  `{:error, :execution_states_unsupported}`. Query your own column directly
...
- `docs/adr/0006-optional-opaque-run-metadata.md` (the file name keeps the
  retired noun; a file name is a cite target), decision 3: metadata
