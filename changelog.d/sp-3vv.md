...
- **Breaking** for a storage adapter outside this package that stores the status or implements `count_executions_by_content_hash/2`: store and read back `:needs_migration`, count it under its own key, and count a durable child's pin while its parent is `:active` or `:needs_migration`. The conformance suite checks all three.
- A delivery to a `:needs_migration` execution through `StatifierPersistence.Executions.step/5` or any `StatifierPersistence.Driver` door answers `{:error, {:needs_migration, execution}}`: nothing is appended, executed or written, and retrying the delivery after the execution leaves the arm is the host's.
