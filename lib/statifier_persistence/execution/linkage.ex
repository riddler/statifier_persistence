defmodule StatifierPersistence.Execution.Linkage do
  @moduledoc """
  ...
  @type t :: %__MODULE__{
          parent_execution_id: Adapter.execution_id(),
          invoke_id: String.t(),
  ...
  def new(parent_execution_id, invoke_id, child_index, content_hash)
      when is_binary(parent_execution_id) and is_binary(invoke_id) and
  ...
  def new(parent_execution_id, invoke_id, child_index, content_hash, child_count, policy)
    ...
    %{
      new(parent_execution_id, invoke_id, child_index, content_hash)
    ...
    case Map.fetch(metadata, reserved_key()) do
      {:ok,
       %{
        ...
        build(parent_execution_id, invoke_id, child_index, content_hash, reserved)
  ...
  defp build(parent_execution_id, invoke_id, child_index, content_hash, reserved) do
    case {Map.fetch(reserved, "child_count"), Map.fetch(reserved, "policy")} do
      {:error, :error} ->
        {:ok, new(parent_execution_id, invoke_id, child_index, content_hash)}

      {{:ok, child_count}, {:ok, policy}} ->
        fan_out(parent_execution_id, invoke_id, child_index, content_hash, child_count, policy)
  ...
  defp fan_out(parent_execution_id, invoke_id, child_index, content_hash, child_count, policy) do
    case Map.fetch(@policies, policy) do
      {:ok, policy} ->
        {:ok, new(parent_execution_id, invoke_id, child_index, content_hash, child_count, policy)}
  ...
  def child_execution_id(parent_execution_id, invoke_id, child_index)
      when is_binary(parent_execution_id) and is_binary(invoke_id) and
  ...
  """
  @spec parent_match(parent_execution_id :: Adapter.execution_id()) :: Adapter.metadata()
  def parent_match(parent_execution_id) when is_binary(parent_execution_id) do
    %{reserved_key() => %{"parent_execution_id" => parent_execution_id}}
  end
  ...
  started.
  """
  @spec invocation_match(parent_execution_id :: Adapter.execution_id(), invoke_id :: String.t()) ::
          Adapter.metadata()
  def invocation_match(parent_execution_id, invoke_id)
      when is_binary(parent_execution_id) and is_binary(invoke_id) do
    %{reserved_key() => %{"parent_execution_id" => parent_execution_id, "invoke_id" => invoke_id}}
  end
end
