if Code.ensure_loaded?(Ecto.Migration) do
  defmodule StatifierPersistence.Ecto.Migrations.V04 do
    ...
    `SHARE` lock on the executions table for the whole build and blocks
    every `INSERT`, `UPDATE` and `DELETE` against it until the build
    finishes. For a host stepping executions durably that is every step of
    every execution. On a small or idle table the build is imperceptible;
    on a large one it is an outage, and the README's answer used to be a hand-written
    ...
    executions table a plain build costs nothing.

    The skip warns through `Logger.warning/1` **only when the executions
    table already holds rows**, which is exactly the case where the plain build
    ...
    `up/1` can leave the executions table with no `metadata` index or with
    an invalid one. Re-running the migration is the repair: the drop is
    ...
    # Only an executions table that already holds rows had anything to
    # lose to V03's plain build, and that is the only case worth a warning: a fresh
    ...
    # commands and runs them at the end: asking the executions table
    # anything from the body of `up/1` reaches it before V01 has created
    # it, on every fresh database. A queued function runs in order, after the
    # DDL ahead of it.
    @spec warn_skipped(Config.t()) :: :ok
    defp warn_skipped(%Config{} = config) do
      repo = repo()
      present_sql = "SELECT EXISTS (SELECT 1 FROM #{qualified_executions(config)} LIMIT 1)"
        ...
        if present? do
          Logger.warning(
            "statifier_persistence V04 skipped the concurrent rebuild of the " <>
              ...
              "plain CREATE INDEX, which held a SHARE lock on the executions " <>
              "table for the whole build. To build it without blocking " <>
              "writes, put @disable_ddl_transaction true and " <>
              "@disable_migration_lock true " <>
    ...
    @spec qualified_executions(Config.t()) :: String.t()
    defp qualified_executions(%Config{prefix: nil} = config) do
      ~s("#{Config.table(config, :executions)}")
    end

    defp qualified_executions(%Config{} = config) do
      ~s("#{config.prefix}"."#{Config.table(config, :executions)}")
    end
